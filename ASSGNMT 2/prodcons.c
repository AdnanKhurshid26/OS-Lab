        ...
        for (int i = 0; i < p; i++)
        {
            ...
            if (producer == 0)
            {
                ...
                while (item_produced < x)
                {
                    ...
                    buff->items[buff->rear] = produce;
                    printf("Producer %d produced %d at %d\n", prod_no, produce, buff->front);
                    if (buff->rear + 1 < BUFFER_LEN)
                        buff->rear++;
                    else if (buff->rear + 1 == BUFFER_LEN)
                        buff->rear = 0;
                    sem_post(buffmutex);
                    item_produced++;
                }
        ...
        for (int i = 0; i < c; i++)
        {
            ...
            if (consumer == 0)
            {
                ...
                while (item_consumed < y)
                {
                    ...
                    int consume = buff->items[buff->front];
                    printf("Consumer %d consumed %d at %d\n", cons_no, consume, buff->rear);
                    buff->items[buff->front] = -1;
                    if (buff->front + 1 < BUFFER_LEN)
                        buff->front++;
                    else if (buff->front + 1 == BUFFER_LEN)
                        buff->front = 0;
                    buff->TOTAL += consume;
